...
class DetailWallpaperViewController: UIViewController {
    ...
    private let viewTitle: UILabel = CocoaViewBuilder.shared.makeLabel(
        text: "미리보기",
        color: .white,
        fontSize: 20,
        fontWeight: .bold,
    ...
    private let wallpaperView: UIImageView = {
        let iv = CocoaViewBuilder.shared.makeImageView(
            image: UIImage(named: "plus.circle"), contentMode: .scaleAspectFit
        )
        CocoaViewBuilder.shared.cropCornerRadius(view: iv)
    ...
    private let placeHolderView: UILabel = {
        let iv = CocoaViewBuilder.shared.makeLabel(
    ...
    lazy var lockScreenView: UIView = {
        ...
        let clock = CocoaViewBuilder.shared.makeLabel(text: "12:30", fontSize: 105, fontWeight: .bold)
        let date = CocoaViewBuilder.shared.makeLabel(text: "12월 30일 금요일", fontSize: 25, fontWeight: .medium)
    ...
    lazy var saveButton: UIButton = {
        let button = CocoaViewBuilder.shared.makeButtonWithTitleAndTarget(title: "앨범에 저장", action: #selector(saveImage), target: self, backgroundColor: .clear)
        button.titleLabel?.font = .systemFont(ofSize: 19, weight: .semibold)
        CocoaViewBuilder.shared.cropCornerRadius(view: button)
        button.alpha = 0
    ...
    lazy var guideLabel: UILabel = {
        let label = CocoaViewBuilder.shared.makeLabel(text: "아래로 내려서 닫기", fontSize: 19, fontWeight: .semibold)
        CocoaViewBuilder.shared.makeLayerShadow(to: label.layer)
        label.alpha = 0
    ...
    lazy var deleteButton: UIButton = {
        ...
        button.titleLabel?.font = .systemFont(ofSize: 19, weight: .semibold)
        button.alpha = 0
        CocoaViewBuilder.shared.makeLayerShadow(to: button.layer, radius: 1)
